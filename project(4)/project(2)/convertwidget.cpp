...
void ConvertWidget::on_Start()
{
    ...
    // Word 转 PDF
    HRESULT r = OleInitialize(0); // 初始化 OLE 框架
    // 创建 Word 应用程序对象
    ...
    // 使用指定的参数打开文档
    QAxObject *doc = pWordDocuments->querySubObject("Open(const QVariant&, const QVariant&, const QVariant&, const QVariant&, const QVariant&, const QVariant&, const QVariant&)",
                                                    filename,           // 文件路径
                                                    confirmconversions, // 不确认转换
                                                    readonly,           // 以只读模式打开
                                                    addtorecentfiles,   // 不添加到最近使用的文件列表
                                                    passworddocument,   // 文档密码
                                                    passwordtemplate,   // 模板密码
                                                    revert);            // 不还原文件
